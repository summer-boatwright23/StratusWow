...
export default function WeatherDetail({ locations }) {
  ...
  const fetchCity = async () => {
    try {
      const urlLA = `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.long}&hourly=apparent_temperature,precipitation_probability,precipitation&models=best_match&daily=apparent_temperature_max,apparent_temperature_min,sunrise,sunset&current_weather=true&temperature_unit=fahrenheit&windspeed_unit=mph&precipitation_unit=inch&timezone=auto`;
      ...
      {weather ? (
        <div>
          ...
          <p>
            ...
            {weather && weather.hourly?.precipitation_probability[0]}%
          </p>
